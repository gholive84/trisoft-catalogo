...
/**
 * Importa produtos do catálogo FACHADA (sunshades) — pasta catalogos/SEPARADOS/FACHADA/.
 *
 * Títulos: "SUNSHADE BOARD", "SUNSHADE FLAPS", "SUNSHADE TUBE", "SUNSHADE PIVOTING",
 * etc. SKU prefix: SS- (todos).
 *
 * Algumas páginas têm muitas variações de SKU (até 30+) com linhas de continuação
 * sem SKU repetido — o parser ignora linhas sem SKU. Admin completa o que precisar.
 *
 * Usa texto extraído COM `-layout`:
 *   pdftotext -layout -enc UTF-8 catalogos/SEPARADOS/FACHADA/facade_*.pdf .../
 *   php scripts/import_facades_pdf.php /tmp/facades/*.txt [--reset]
 */
...
if ($files === []) {
    fwrite(STDERR, "Uso: php scripts/import_facades_pdf.php <file.txt|dir|glob> [--reset]\n");
    exit(1);
}
...
if ($reset) {
    fwrite(STDOUT, "⚠️  --reset: apagando produtos da categoria 'fachadas'...\n");
    $pdo->exec("SET FOREIGN_KEY_CHECKS=0");
    $pdo->exec("
        DELETE FROM product_images WHERE product_id IN (
            SELECT pc.product_id FROM product_categories pc
              JOIN categories c ON c.id = pc.category_id
             WHERE c.slug = 'fachadas'
        )
    ");
    $pdo->exec("
...
function ensureCategoryFacades(\PDO $pdo, string $slug, string $name, ?int $parentId = null): int
{
...
function slugifyFacades(string $s): string
{
...
$fachadasId = ensureCategoryFacades($pdo, 'fachadas', 'Fachadas');
...
fwrite(STDOUT, "\nEncontrados " . count($products) . " produtos únicos em FACHADA.\n\n");
...
foreach ($products as $slug => $p) {
    ...
    $familySlug = 'fachadas-' . slugifyFacades($p['title']);
    $familyCatId = ensureCategoryFacades($pdo, $familySlug, ucwords(strtolower($p['title'])), $fachadasId);

    $description = "Brise solar / fachada Trisoft.\n\n"
    ...
    $shortDesc = "Brise / fachada acústica — " . strtolower($p['title']) . ".";
